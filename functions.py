...
def json_to_biomechanics2(data):
    bone_list = []
    positions = []
    orientations = []
    bodies = []
    ...
    for index, row in df_ori.iterrows():
        ...
        for i in range(len(row)):
            r = R.from_quat(row[i])
            er = r.as_euler("xyz", degrees=False)
            tmp.append(er)
        ori_adj.append(np.array(tmp).flatten())
    ori_adj_df = pd.DataFrame(ori_adj)

    ori_adj_df.columns = b_index
    ori_adj_df.to_csv("test_testdeg.csv")
    return df_pos,ori_adj_df


def create_dataset_SMPL(file, smpl=None, jsonfile=None):
    dataset = pd.DataFrame({
        'left_knee_flexion': get_flexion("left", file),
        'right_knee_flexion': get_flexion("right", file),
        'left_hip_abduction': get_abduction('left', file),
    ...
    dataset['knee_acceleration_left'] = get_acceleration_new(dataset["knee_velocity_left"])
    dataset['knee_acceleration_right'] = get_acceleration_new(dataset["knee_velocity_right"])
    dataset['pelvis_acceleration'] = get_acceleration_new(dataset["pelvis_velocity"])
    dataset['elbow_acceleration_left'] = get_acceleration_new(dataset["elbow_velocity_left"])
    dataset['elbow_acceleration_right'] = get_acceleration_new(dataset["elbow_velocity_right"])
    if smpl is not None:
        pose_df_SMPL = pose_from_smpl(joblib.load(smpl))
        for column in pose_df_SMPL:
            for j in range(len(pose_df_SMPL[column])):
                pose_df_SMPL[column][j] = math.degrees(pose_df_SMPL[column][j])

        pose_df_SMPL = pose_df_SMPL.add_suffix("_rotation")
        pose_df_SMPL['index'] = range(len(pose_df_SMPL['knee_l_x_rotation']))
        pose_df_SMPL.set_index(pose_df_SMPL['index'])
        pos_df_SMPL = joints_from_smpl(joblib.load(smpl))
        pos_df_SMPL = pos_df_SMPL.add_suffix("_position")
        pos_df_SMPL['index'] = range(len(pos_df_SMPL['knee_l_x_position']))
        pos_df_SMPL.set_index(pos_df_SMPL['index'])
        concat_df = combine_datasets(dataset, pose_df_SMPL, pos_df_SMPL)

    elif jsonfile is not None:
        f = open(jsonfile)
        pos_df_JSON, pose_df_JSON = json_to_biomechanics2(json.load(f))
        pos_df_JSON = pos_df_JSON.add_suffix("_position")
        pos_df_JSON['index'] = range(len(pos_df_JSON['knee_l_x_position']))
        pos_df_JSON.set_index(pos_df_JSON['index'])
        for column in pose_df_JSON:
            for j in range(len(pose_df_JSON[column])):
                pose_df_JSON[column][j] = math.degrees(pose_df_JSON[column][j])
        pose_df_JSON = pose_df_JSON.add_suffix("_rotation")
        pose_df_JSON['index'] = range(len(pose_df_JSON['knee_l_x_rotation']))
        pose_df_JSON.set_index(pose_df_JSON['index'])
        concat_df = combine_datasets(dataset, pose_df_JSON, pos_df_JSON)
    return concat_df
